...
const vehicleViews = ["Front 3/4", "Side Profile", "Rear 3/4", "Interior", "Detail", "Overhead"];
...
function CarSilhouette({ color }: { color: string }) {
  return (
    ...
    >
      ...
      <path d="M160 300 C160 265 185 245 220 245 C255 245 280 265 280 300" stroke={color} strokeWidth="20" strokeLinecap="round" fill="none" />
      <path d="M620 300 C620 265 645 245 680 245 C715 245 740 265 740 300" stroke={color} strokeWidth="20" strokeLinecap="round" fill="none" />
      <circle cx="220" cy="310" r="52" fill="#1A1A1A" />
      <circle cx="220" cy="310" r="38" fill="#2A2A2A" />
      <circle cx="220" cy="310" r="18" fill="#D0D0D0" />
      {[0, 60, 120, 180, 240, 300].map((deg) => (
        <line key={deg} x1={220 + 18 * Math.cos((deg * Math.PI) / 180)} y1={310 + 18 * Math.sin((deg * Math.PI) / 180)} x2={220 + 36 * Math.cos((deg * Math.PI) / 180)} y2={310 + 36 * Math.sin((deg * Math.PI) / 180)} stroke="#B0B0B0" strokeWidth="3" strokeLinecap="round" />
      ))}
      ...
      >
        <AnimatePresence mode="wait">
          <motion.div
            ...
            style={{ padding: isMobile ? "16px 16px 0" : "0 24px" }}
          >
